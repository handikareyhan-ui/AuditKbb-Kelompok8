...
const API_BASE_URL = "http://localhost:3000/api";
  ...
  // TAMBAHKAN KODE BERIKUT:
    if (window.AuditMap && window.AuditMap.map) {
        window.AuditMap.map.jumpTo({
            center: [107.4475, -6.8450], // Koordinat tengah KBB
            zoom: 10.2                   // Level zoom yang pas
        });
      }
  }
      ...
      ? items
          .map((item) => {
            ...
            return (
              ...
              `<td class="pkg">` +
                // --- Bagian Deteksi Anomali ---
              (item.severity && item.severity.toLowerCase().includes('high') ? 
                `<div style="margin-bottom: 6px;"><span style="background:#ef4444; color:white; padding:3px 8px; border-radius:4px; font-size:11px; font-weight:bold;">⚠️ ANOMALI ANGGARAN</span></div>` : ''
              ) + 

              // --- Bagian Potensi UMKM ---
              (item.budget < 200000000 && item.budget > 0 ? 
                `<div style="margin-bottom: 6px;"><span style="background:#22c55e; color:white; padding:3px 8px; border-radius:4px; font-size:11px; font-weight:bold;">🏢 POTENSI UMKM KBB</span></div>` : ''
              ) + 

                `<div style="font-weight:500;">${escapeHtml(item.packageName)}</div>` +
              `</td>` +
 ...
 // Fungsi Inovasi: Filter Paket untuk UMKM Lokal (Poin 3c)
window.filterUMKM = function() {
    console.log("Menjalankan filter UMKM Bandung Barat...");

    // 1. Jalankan fungsi setMapFilter untuk memastikan state aplikasi sinkron
    // 'pemkot' digunakan agar filter kategori beralih ke level Pemerintah Kota/Kabupaten
    if (typeof setMapFilter === "function") {
        setMapFilter('pemkot'); 
    }

    // 2. Masukkan kata kunci ke kolom pencarian (Logika dari image_b6eed1.png)
    const searchInput = document.querySelector('input[type="text"]') || document.getElementById('search-input');
    if (searchInput) {
        searchInput.value = "Bandung Barat";
        searchInput.dispatchEvent(new Event('input', { bubbles: true }));
    }

    // 3. Tambahkan efek Visual (Opsional: Zoom ke peta)
    if (window.AuditMap && window.AuditMap.map) {
        window.AuditMap.map.flyTo({
            center: [107.4475, -6.8450], // Koordinat KBB
            zoom: 10,
            essential: true
        });
    }

    alert("Menampilkan potensi proyek untuk UMKM di Bandung Barat (Anggaran < 200 Juta)");
};
